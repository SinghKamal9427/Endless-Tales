...
import StepF from "../../public/assets/step1.png";
import SciFi from "../../public/assets/Sci-Fi-icon.png";
import storyTypesData from "../utilis/storyTypesData"
import { useContext } from "react";
import { Context } from "../store/createContextStore";


export default function StepFirst ({ onNext}) {

  const { data , setData} = useContext(Context)

  const handleSelect= (val) => {
    setData(()=>[{ title: val.title , id:val.id }])
    onNext()
  }

    return (
        <div className="flex flex-col items-center justify-center cursor-pointer" >
        <Image src={StepF} alt="Hero Banner" className="w-[10em] h-[7em] " />
        <div className="font-black text-2xl pb-8">{storyTypesData[0].chooseType}</div>
        <div className="flex flex-wrap items-center justify-center gap-4 md:mx-[26%]">
        {storyTypesData.map((val,inx)=>{
