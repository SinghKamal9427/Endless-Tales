...
import StepTh from "../../public/assets/step3.png";
import SciFi from "../../public/assets/Sci-Fi-icon.png";
import storyArtifactsData from "../utilis/storyArtifactsData";
import { useContext } from "react";
import { Context } from "../store/createContextStore";

export default function StepThird({ onNext }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      <Image src={StepTh} alt="Hero Banner" className="w-[10em] h-[7em] " />
      <div className="font-black text-2xl pb-8">
        {storyArtifactsData[0].chooseType}
      </div>
      <div className="flex flex-wrap items-center justify-center gap-4 md:mx-[26%]">
        {storyArtifactsData.map((val, inx) => {
          return (
            <div
              key={inx}
              className="h-[145px] w-[145px] bg-[#ffffff34] group  hover:bg-[#02b7ff] transition-all duration-100 rounded-[30px]  "
            ...
            >
              ...
              <div className="h-[40%] flex flex-col items-center justify-center text-center px-2">
                <div className=" group-hover:text-white text-black text-sm font-bold ">
                  {" "}
                  {val.title}{" "}
                </div>
              </div>
